...
public class JOptionPaneDemo1 {
	...
	public void init() {
		...
		messageTypePanel = new ButtonPanel("选择消息的类型", new String[] { "ERROR_MESSAGE", "INFORMATION_MESSAGE",
				"WARNING_MESSAGE", "QUESTION_MESSAGE", "PLAIN_MESSAGE" });// 创建消息类型面板
		...
		confirmPanel = new ButtonPanel("确认对话框",
				new String[] { "DEFAULT_OPTION", "YES_NO_OPTION", "YES_NO_CANCEL_OPTION", "OK_CANCEL_OPTION" });// 包含四个选项
	...
	private Object[] getOptions() {
		if ("字符串选项".equals(optionsPanel.getSelection())) {
			...
			return new Icon[] { new ImageIcon(Const.classPath + "images/swing/1.gif"),
					new ImageIcon(Const.classPath + "images/swing/2.gif"),
					new ImageIcon(Const.classPath + "images/swing/3.gif"),
					new ImageIcon(Const.classPath + "images/swing/4.gif") };
		} else {
		...
		public void actionPerformed(ActionEvent e) {
			if ("确认对话框".equals(e.getActionCommand())) {
				...
				if ("单行文本框".equals(inputPanel.getSelection())) {
					...
					JOptionPane.showInputDialog(frame, getMessage(), "输入对话框", getDialogType(), null,
							new String[] { "轻量级JavaEE企业应用开发", "Struts2权威指南" }, "Struts2权威指南");
				}
				...
				JOptionPane.showOptionDialog(frame, getMessage(), "选项对话框", getOptionType(), getDialogType(), null,
						getOptions(), "b");
			}
		}
	}
